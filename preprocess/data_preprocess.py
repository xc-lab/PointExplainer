#  -*- coding: utf-8 -*-
'''
DATA_PREPROCESSING Step II:

point-cloud construction and feature augmentation
for dynamic handwriting data in Parkinson's disease analysis.

This script reads raw handwriting recordings for each cross-validation fold,
computes derived kinematic features (e.g., velocity, acceleration, jerk,
curvature), and exports the resulting trajectories as point clouds.
...
def get_aug_dataset(path, dataset, fold_d, dimension, hw_pattern_list):
    '''
    Processes data with generating point clouds for Parkinson's disease analysis.
...
if __name__ == '__main__':
    ...
    hw_pattern_list = 0 # 0: Static Spiral Test (SST Dataset) ;  1: Dynamic Spiral Test (DST Dataset)

    for fold_d in ['fold_1', 'fold_2', 'fold_3']:
        print("******{}******".format(fold_d))
        get_aug_dataset(path, dataset, fold_d, dimension, hw_pattern_list)
